...
| [`finance_strategy`](finance/finance_strategy/README.md) | Designing (`CG-SWING` daily slice) | Shared research and trade-plan language | **`CG-SWING` Resolved — 2026-08-06; `CG-RISK`, `CG-DAY`, and `CG-QUANT` remain Open.** Pure falsifiable hypotheses, point-in-time universes, versioned predicates, desired entry/invalidation/target/expiry, evidence readiness, signal evaluation, and planned-versus-observed transitions. The first slice can distinguish `SetupQualified` from `NotReady`; it cannot produce a fully `Accepted` sized/executable plan until the adjacent receipts exist. |
...
The three trader-decision packages extend the acyclic graph in that order of
...
execution do not import one another; Pi workbenches compose them. None imports
Pi, HTTP, a provider adapter, or a market-owned package.
