...
const liveConfirmation = "I_ACCEPT_ONE_FUTU_US_RIGHTS_QUERY";
const quoteRightMeanings = Object.freeze({
  0: "unknown",
  1: "bmp_subscription_unavailable",
  2: "level_1",
  3: "level_2",
  4: "hk_sf_not_applicable_to_us",
  5: "no_permission",
  6: "level_3",
});

export function validateRightsConfig(argv, environment) {
...
export function allowlistedRightsEvidence(response) {
  const value = response?.s2c?.usQotRight;
  if (!Number.isInteger(value) || quoteRightMeanings[value] === undefined) {
    throw new Error("malformed_us_quote_right");
  }
  return {
    schemaVersion: 1,
    kind: "futu_us_quote_right_probe",
    status: "passed",
    provider: "Futu OpenD",
    ...
    usQuoteRight: {
      value,
      meaning: quoteRightMeanings[value],
...
async function run() {
  try {
    validateRightsConfig(process.argv.slice(2), process.env);
  } catch (error) {
    return failed(error);
  }
  ...
  try {
    ...
    return allowlistedRightsEvidence(response);
  } catch (error) {
    return failed(error);
  } finally {
    if (client?.websock) {
