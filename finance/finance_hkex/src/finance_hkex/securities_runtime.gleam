...
pub fn new(_access: Access) -> Result(Runtime, InitError) {
  authority_runtime.new_binary(policy()) |> map_result(Runtime, InvalidRuntime)
}
...
pub fn new_with(
  ...
  authority_runtime.new_binary_with(policy(), sender, sleeper, clock)
...
fn policy() -> authority_runtime.Policy {
  ...
  let assert Ok(value) =
    authority_runtime.policy(
      origin: hkex_request.securities_origin,
      allowed_paths: [hkex_request.full_list_path],
