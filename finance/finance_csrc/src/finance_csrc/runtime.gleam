...
import finance_csrc.{type Access}
import finance_csrc/request as csrc_request
...
fn policy() -> authority_runtime.Policy {
  ...
  let assert Ok(value) =
    authority_runtime.policy(
      origin: csrc_request.origin,
      allowed_paths: [
        csrc_request.market_monthly_path,
        csrc_request.market_weekly_path,
        csrc_request.consultation_feedback_path,
      ],
