...
pub const schema_name = "pi-sparkles/hk-ohlcv-gap-receipt"
...
pub fn listing(value: Receipt) -> hk_identity.Listing {
  value.listing
}

pub fn board(value: Receipt) -> hk_identity.Board {
  value.listing |> hk_identity.board
}

pub fn share_class(value: Receipt) -> hk_identity.ShareClass {
  value.listing |> hk_identity.share_class
}

pub fn currency(value: Receipt) -> Currency {
  value.listing |> hk_identity.currency
}

pub fn code(value: Receipt) -> String {
  value.listing |> hk_identity.code
}
...
pub fn board_name(value: hk_identity.Board) -> String {
  case value {
    hk_identity.MainBoard -> "main"
    hk_identity.Gem -> "gem"
  }
}

pub fn share_class_name(value: hk_identity.ShareClass) -> String {
  case value {
    hk_identity.OrdinaryShare -> "ordinary_share"
    hk_identity.DepositaryReceipt -> "depositary_receipt"
  }
}
...
fn identity_fields(
  ...
  use venue <- result.try(field("venue", venue_name()))
  use board <- result.try(field("board", board_name(hk_identity.board(listing))))
  use share_class <- result.try(field(
    "share_class",
    share_class_name(hk_identity.share_class(listing)),
  ))
  use currency <- result.try(field(
    "currency",
    listing |> hk_identity.currency |> currency.code,
  ))
  use code <- result.try(field("code", hk_identity.code(listing)))
