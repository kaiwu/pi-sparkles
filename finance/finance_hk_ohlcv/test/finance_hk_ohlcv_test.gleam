...
pub fn package_is_experimental_test() {
  finance_hk_ohlcv.status() |> should.equal(finance_hk_ohlcv.Experimental)
}

pub fn hk_calendar_classifies_closure_suspension_and_omission_test() {
  let assert Ok(suspended) =
    gap_assessment.status_receipt(
      civil(2026, 6, 22),
      gap_assessment.Suspended,
      "authority:hkex-halt:00700:2026-06-22",
    )
  let assert Ok(trading) =
    gap_assessment.status_receipt(
      civil(2026, 6, 23),
      gap_assessment.Trading,
      "authority:hkex-status:00700:2026-06-23",
    )
  let assert Ok(value) =
    assessment.assess(
  ...
  let assert [tuen_ng, saturday, sunday, suspension, omission] = gaps
  gap_assessment.gap_state(tuen_ng)
...
pub fn incomplete_or_unexplained_hk_receipts_fail_closed_test() {
...
pub fn hk_canonical_receipt_binds_identity_page_and_bar_dates_test() {
  let first =
    gap_projection(
      [civil(2026, 6, 18), civil(2026, 6, 24)],
      string.repeat("a", 64),
  ...
  gap_receipt.board(first) |> should.equal(hk_identity.MainBoard)
...
fn listing_receipt(starts: time.Date) -> assessment.ListingReceipt {
  ...
  let assert Ok(value) =
    assessment.listing_receipt(
      listing(),
      interval,
      "authority:listing:00700:XHKG:2026",
    )
  value
}

fn listing() -> hk_identity.Listing {
  let assert Ok(instrument_id) = identifier.instrument_id("hkex:00700")
  let assert Ok(hkd) = currency.from_code("HKD")
  let assert Ok(value) =
    hk_identity.new(
      instrument_id,
      "00700",
      hk_identity.MainBoard,
