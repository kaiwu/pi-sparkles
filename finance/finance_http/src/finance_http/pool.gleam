import finance_http/client.{type Client, type ClientError}
...
/// A small effect owner around the pure `scheduler` reducer.
///
/// The mutable cell and abort listeners live behind a narrow JavaScript FFI;
/// all capacity, fairness, queueing, and cancellation state transitions remain
/// immutable Gleam values.
...
/// Submit one request and resolve when it completes, is cancelled, or cannot
/// be admitted. The transition into active or waiting state is synchronous, so
/// callers cannot overrun the limits by submitting several promises together.
pub fn send(
  pool: Pool,
  id id: String,
...
/// Cancel a waiting or active call by id.
///
/// Waiting calls leave the queue and resolve immediately. Active calls retain
/// their concurrency slot until their client promise observes the abort and
/// settles, preventing cancellation from briefly exceeding configured limits.
pub fn cancel(pool: Pool, id id: String) -> Result(Nil, SchedulerError) {
  ...
  case cancellation {
    scheduler.CancelledWaiting(job) -> {
      ...
      resolve(Error(RequestFailed(client.Cancelled)))
    }
...
fn launch(pool: Pool, job: scheduler.Job(Call)) -> Nil {
  ...
  let _ =
    client.send(client_value, request_value, cancellation)
