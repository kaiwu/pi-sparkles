...
import finance_cninfo.{type Access}
import finance_cninfo/request as cninfo_request
...
pub fn new(_access: Access) -> Result(Runtime, InitError) {
  authority_runtime.new(policy()) |> map_result(Runtime, InvalidRuntime)
}
...
fn policy() -> authority_runtime.Policy {
  ...
  let assert Ok(value) =
    authority_runtime.policy(
      origin: cninfo_request.discovery_origin,
      allowed_paths: [
        cninfo_request.security_master_path,
        cninfo_request.announcement_query_path,
      ],
