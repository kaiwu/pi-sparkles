...
The current target has no provider decision: `pi_stock_technicals` consumes
caller/LLM-supplied exact bar and receipt inputs and reuses
`finance_indicators`. The LLM chooses which query to make, which supplied inputs
and parameters to use, which calculation or drill-down to request, how to
interpret the returned evidence, and what to do next. The plugin supplies only
the requested facts, evidence, calculations, provenance, unknowns, and exact
operation failures. Its next step is detailed plugin design under Sessions 12
and 17.
