...
| `pi_finance_dataset` | P1 | **`CG-MARKET-DATA` daily information slice Resolved — 2026-08-07; general import adapters remain incremental.** Explicit, bounded import/profile/query/export for user-selected CSV, JSON, XLSX, and read-only SQL sources. It preserves raw lexemes, schema mappings, query/as-of boundaries, provenance, unknowns/conflicts, and manifests; credentials stay opaque and user data is not promoted to provider or authority evidence. | `/dataset`, `dataset_import`, `dataset_profile`, `dataset_query`, `dataset_manifest` |
...
| `pi_trade_plan` | P1 | **`CG-RISK` calculation slice and `CG-SWING` daily interface Resolved — 2026-08-07; consuming persona gates still apply.** A read-only, versioned declaration joining exact listing/track, thesis or setup, entry trigger, invalidation/stop, targets, holding horizon, caller-selected account risk budget, lot/tick facts, requested position-size calculations, reward/risk, scaling, and evidence freshness. It proposes no order and reports every coherence, age, unknown, and conflict fact for the LLM to decide. | `/trade-plan`, `trade_plan_create`, `position_size`, `risk_reward`, `trade_plan_validate` |
| `pi_order_simulator` | P1 | **The execution-information slice of `CG-DAY` and shared-replay slice of `CG-QUANT` are resolved; the full intraday `CG-DAY` workflow remains Open.** Deterministic market/limit/stop/stop-limit what-if evaluation over explicit bar, quote, book, or tape evidence and a caller-selected `finance_execution` fill/cost model. Ambiguous intrabar ordering, gaps, partial fills, latency, impact, and queue position remain ranges or unknowns rather than favorable fills. | `/order-sim`, `order_simulate`, `order_compare`, `execution_cost_estimate` |
...
| `pi_quant_research` | P1 | **`CG-QUANT` shared-replay information slice Resolved — 2026-08-07; first pure core Experimental.** Falsifiable LLM/user hypothesis registration plus point-in-time dataset/universe manifests, shared feature/signal definitions, caller-declared train/validation/test or walk-forward plans, run comparison, complete trial accounting, explicitly requested statistical calculations, and reproducible exports. It supplies information and operations without research verdicts. | `/quant`, `research_hypothesis`, `dataset_manifest`, `validation_plan`, `research_run_compare` |
...
| `pi_portfolio_risk` | P1 | **`CG-RISK` calculation slice Resolved; `CG-PORTFOLIO` remains Open.** Caller-requested position and portfolio calculations for budgets, gross/net and leverage exposure, concentration, volatility, beta, correlation, VaR/CVaR, liquidity, currency, drawdown and losing-streak facts with dated inputs, method labels, declared limits, and contributors. It chooses no policy, threshold, scenario, or response. | `/risk`, `portfolio_risk`, `risk_budget`, `risk_contributors`, `correlation_matrix` |
...
| `pi_backtest` | P1 | **`CG-QUANT` shared-replay information slice Resolved — 2026-08-07; `finance_replay` first core Experimental.** Reproducible event/bar tests consuming shared strategy and execution definitions, point-in-time identity/universes, corporate actions, calendar/session rules, data-availability lags, fees/taxes/spread/slippage, and explicit fill ambiguity. Caller-declared partitions, trial accounting, calculations, benchmark/cash treatment, temporal facts, and dataset/run manifests remain inspectable; only the LLM interprets them. | `/backtest`, `backtest_run`, `backtest_compare`, `backtest_audit` |
