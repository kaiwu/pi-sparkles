...
| Proposed plugin | Priority | What it gives Pi | Candidate commands/tools |
| --- | --- | --- | --- |
| `pi_stock_quote` (**Selected next — provider-neutral thin shell**) | P0 | Inspect one exact caller/provider-adapter quote observation for an explicit `cn`, `hk`, or `us` listing, preserving bid/ask lexemes, sizes, timestamps, feed, entitlement, licence, source receipt, and unknowns. The first slice composes `finance_quote` without fetching, selecting, or falling back among providers. | `stock_quote`; later `/quote`, provider acquisition, `stock_quotes` |
