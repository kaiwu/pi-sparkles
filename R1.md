...
The next two Session 17 breadth slices are also real code:
...
daily series, each for an exact `cn`, `hk`, or `us` listing/MIC. Both retain
source lexemes, timing/evidence, entitlement, licence, and explicit unknowns
without selecting or calling a provider.
...
| `pi_stock_market_snapshot` (**Selected next — provider-neutral point-in-time breadth slice**) | P1 | Point-in-time index/sector/industry breadth, leaders, laggards, gaps, volume, volatility, and explicitly defined relative-strength/rotation views without treating fund-flow proxies as observed flows. | `/market`, `market_snapshot`, `market_breadth`, `sector_rotation`, `market_movers` |
