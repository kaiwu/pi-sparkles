...
| [`pi_swing_workbench`](plugins/swing_workbench/README.md) | P1 · Designing (`CG-SWING` shell) | **`CG-SWING` Resolved — 2026-08-06; `CG-MARKET-DATA`, `CG-RISK`, `CG-TECH`, `CG-DAY`/execution, and `CG-PSYCHOLOGY`/journal prerequisites remain Open.** A professional daily rhythm: after-close change/exception triage, inspectable facts, LLM-authored next-session plan declaration and preflight evidence, monitoring by exception, and planned-versus-observed review. The shell reports every missing/incompatible fact and never makes the setup or trade decision. | `/swing`, `swing_candidates`, `swing_plan`, `swing_review` |
