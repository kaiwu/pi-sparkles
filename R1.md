...
and [`R2.md`](R2.md#breadth-first-policy-and-active-queue). Ranks 1 through 19
...
the Session 21 bounded raw CSV/JSON portfolio import-and-inspection slice. The
full portfolio review workflow remains gated, and rank 20 full `CG-DAY` is the
next course stop.
...
| [`pi_day_workbench`](plugins/day_workbench/README.md) (**Designing — Session 22 full workflow Resolved 2026-08-09**) | P2 | Network-free validation and inspection of caller-supplied licence-attested sequenced intraday packets, explicit LLM-selected calculations, and stateless content-bound workflow transitions. It preserves exact session/feed/identity/entitlement/licence/sequence/freshness/gap facts and never verifies caller claims, fetches a provider, decides readiness/trades, predicts fills, persists state, or mutates orders/accounts. | `day_inspect`, `day_calculate`, `day_transition`; later `/day`, provider acquisition, scans, durable monitoring, alerts, paper/live mutation |
