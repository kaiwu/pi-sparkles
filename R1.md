...
| `pi_trade_plan` | P1 | **Course gates `CG-RISK` plus the consuming persona gate (Open).** A read-only, versioned plan joining exact listing/track, thesis or setup, entry trigger, invalidation/stop, targets, holding horizon, account risk budget, lot/tick rules, position size, reward/risk, scaling, and evidence freshness. It proposes no order and refuses incoherent or stale inputs. | `/trade-plan`, `trade_plan_create`, `position_size`, `risk_reward`, `trade_plan_validate` |
...
| `pi_swing_workbench` | P1 | **Course gates `CG-SWING`, `CG-RISK`, and `CG-TECH` (Open).** End-to-end daily/weekly research: point-in-time screen, technical/sector/catalyst confirmation, inspectable setup, risk-sized plan, watch/alert state, and planned-versus-actual journal review. This is the first persona workflow target. | `/swing`, `swing_candidates`, `swing_plan`, `swing_review` |
