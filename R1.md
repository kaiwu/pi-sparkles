...
The next Session 17 breadth slice is also real code:
`pi_stock_quote.stock_quote` constructs one canonical provider-neutral quote
observation for an exact `cn`, `hk`, or `us` listing/MIC while retaining source
lexemes, time, feed, evidence ID, entitlement, licence, and explicit unknowns.
...
| `pi_stock_history` (**Selected next — provider-neutral daily slice**) | P0 | Inspect one exact bounded caller/provider-adapter daily OHLCV series for an explicit `cn`, `hk`, or `us` listing/MIC, preserving raw lexemes, adjustment, calendar/gap facts, source receipts, entitlement, licence, and unknowns without choosing or fetching a provider. | `stock_bars`; later `/chart-data`, provider acquisition, intraday, `stock_returns`, `stock_performance` |
