...
and [`R2.md`](R2.md#breadth-first-policy-and-active-queue). Ranks 1 through 12,
`pi_stock_technicals`, `pi_finance_sources`, `pi_trade_plan`,
`pi_order_simulator`, `pi_finance_dataset`, the `pi_stock_screener` predicate
increment, `pi_finance_calendar`, `pi_quant_research`, `pi_backtest`, and
`pi_finance_charts`, the HK-only `pi_stock_earnings_calendar` source slice, and
the US-only `pi_stock_corporate_actions` source slice are now Experimental.
Rank-13 `pi_portfolio_risk` is paused at its light `CG-PORTFOLIO` course stop.
...
| `pi_portfolio_risk` (**Course stop — Session 17 rank 13; tutor input requested 2026-08-09**) | P1 | **`CG-RISK` calculation slice Resolved; light `CG-PORTFOLIO` OpenBlocking for the multi-position contract.** The proposed first slice is single-currency heat/exposure and per-position contribution under explicit caller/LLM-selected formulas, with no optimization or rebalancing. `/tmp/QA01.md` asks the expert to define inputs, denominators, temporal coherence, partial-information behavior, reconciliation, and the plugin/LLM judgment boundary before design. | `/risk`, `portfolio_heat`, `exposure`; later `portfolio_risk`, `risk_contributors`, `correlation_matrix` |
