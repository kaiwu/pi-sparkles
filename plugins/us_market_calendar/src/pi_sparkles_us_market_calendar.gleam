...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    ...
    "us_market_calendar",
    "US market calendar",
    "Classify one date with the exchange-published 2026 NYSE or Nasdaq regular-equity schedule; preserve source, MIC, coverage, early closes, entitlement, and limitations",
    "Check whether an exact US equity venue is scheduled to trade on a 2026 date",
    ...
    fn(_id, input, _signal, _updates, _ctx) {
      case query.run(venue: input.venue, on: input.date) {
        Error(query.OutsideCoverage(_)) ->
          tool.reject(
            "US official calendar covers only 2026; no weekday fallback was used",
          )
        Error(_) -> tool.reject("US official calendar query was invalid")
        Ok(value) ->
...
fn input_schema() -> schema.Schema {
  schema.object([
    schema.Required(
      "venue",
      schema.string_enum(["nyse", "nasdaq"])
        |> schema.described(
          "Exact US exchange venue; never inferred from symbol",
        ),
    ),
    schema.Required(
      "date",
...
fn input_decoder() -> decode.Decoder(Input) {
  ...
  let placeholder = Input(query.Nyse, placeholder_date)
  case query.venue_from_name(venue_text), parse_date(date_text) {
    Ok(venue), Ok(date) -> decode.success(Input(venue, date))
    _, _ -> decode.failure(placeholder, "valid US venue and Gregorian date")
  }
}
...
fn render(value: query.ResultValue) -> String {
  ...
  <> " official 2026 regular-equity calendar | "
...
fn open_kind(sessions: List(calendar.Session)) -> String {
  case sessions {
    [session] -> {
      let #(hour, minute) = local.time_parts(calendar.closes_at(session))
