...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "fixed_income_analyze",
    "Analyze one exact fixed-income instrument",
    "Validate one exact bond identity, conventions, cash flows and curve, then execute caller-selected price, yield, duration, convexity, DV01 and same-convention G-spread calculations",
    "The caller owns curve, solver, interpolation and interpretation; no extrapolation, convention coercion, valuation verdict or recommendation occurs",
...
fn finish(
  ...
  case outcome {
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "Fixed-income packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("Fixed-income import was cancelled")
    finance_local_import.Missing ->
      tool.reject("Fixed-income packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("Fixed-income packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("Fixed-income import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("Fixed-income import returned an invalid effect result")
  }
}
