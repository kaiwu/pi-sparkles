...
import pi/schema
import pi/tool
import pi_sparkles_hk_stock_shareholders/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "hk_shareholder_notices",
    "Inspect HK substantial-shareholder notices",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "hk_shareholder_notice_record",
    "Drill one HK substantial-shareholder notices record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by hk_shareholder_notices; no latest-record or preferred-source choice is made",
