...
import pi/schema
import pi/tool
import pi_sparkles_portfolio_rebalance/domain
...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "compute_rebalance",
    "Compute mechanical rebalance deltas",
    "Compute continuous and caller-grid quantity/value deltas from an exact portfolio receipt and target receipt, retaining track legs, cash projection, turnover, and constraint facts",
    "The result is neither a recommendation nor an order; the caller owns targets, grids, constraints, interpretation, authorization, sequencing, and execution",
...
fn finish(
  ...
  case outcome {
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "Rebalance packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("Rebalance import was cancelled")
    finance_local_import.Missing ->
      tool.reject("Rebalance packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("Rebalance packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("Rebalance import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("Rebalance import returned an invalid effect result")
  }
}
