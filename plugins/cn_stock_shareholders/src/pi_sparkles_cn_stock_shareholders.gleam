...
import pi/schema
import pi/tool
import pi_sparkles_cn_stock_shareholders/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_shareholders",
    "Inspect CN shareholder disclosures",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_shareholder_record",
    "Drill one CN shareholder disclosures record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_shareholders; no latest-record or preferred-source choice is made",
