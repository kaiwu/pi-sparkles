...
import pi/schema
import pi/tool
import pi_sparkles_finance_catalysts/domain
...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "catalyst_timeline",
    "Build exact cross-source event timeline",
    "Validate and order exact filing, earnings, corporate-action, news and caller-event receipts for one listing and range while preserving independent reports, equal times, corrections, retractions, rights, coverage and partial failures",
    "Catalyst is a caller workflow label only; this timeline assigns no importance, direction, causality, probability, recommendation, alert, or action",
...
fn finish(
  ...
  case outcome {
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "Catalyst receipt packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("Catalyst receipt import was cancelled")
    finance_local_import.Missing ->
      tool.reject("Catalyst receipt packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("Catalyst receipt packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("Catalyst receipt import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("Catalyst receipt import returned an invalid effect result")
  }
}
