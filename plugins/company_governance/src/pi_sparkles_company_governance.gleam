...
import pi/schema
import pi/tool
import pi_sparkles_company_governance/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "company_governance",
    "Inspect Company governance evidence",
    "Read and validate one exact content-bound us research packet from a caller-owned regular UTF-8 file; return compact source facts, omissions, and stable record handles without interpretation",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "company_governance_record",
    "Drill one Company governance evidence record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by company_governance; no latest-record or preferred-source choice is made",
