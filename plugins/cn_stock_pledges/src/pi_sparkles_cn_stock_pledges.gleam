...
import pi/schema
import pi/tool
import pi_sparkles_cn_stock_pledges/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_share_pledges",
    "Inspect CN share pledge events",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_share_pledge_record",
    "Drill one CN share pledge events record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_share_pledges; no latest-record or preferred-source choice is made",
