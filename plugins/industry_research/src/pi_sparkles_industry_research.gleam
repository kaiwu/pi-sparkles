...
import pi/schema
import pi/tool
import pi_sparkles_industry_research/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "industry_research",
    "Inspect Industry research evidence",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "industry_research_record",
    "Drill one Industry research evidence record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by industry_research; no latest-record or preferred-source choice is made",
