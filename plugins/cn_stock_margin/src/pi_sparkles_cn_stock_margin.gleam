...
import pi/schema
import pi/tool
import pi_sparkles_cn_stock_margin/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_margin_records",
    "Inspect CN financing and securities-lending facts",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_margin_record",
    "Drill one CN financing and securities-lending facts record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_margin_records; no latest-record or preferred-source choice is made",
