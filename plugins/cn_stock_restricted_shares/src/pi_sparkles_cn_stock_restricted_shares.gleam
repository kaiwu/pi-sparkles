...
import pi/schema
import pi/tool
import pi_sparkles_cn_stock_restricted_shares/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_restricted_shares",
    "Inspect CN restricted-share events",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_restricted_share_record",
    "Drill one CN restricted-share events record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_restricted_shares; no latest-record or preferred-source choice is made",
