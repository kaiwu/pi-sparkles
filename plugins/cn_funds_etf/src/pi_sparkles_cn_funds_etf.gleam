...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "cn_fund_etf_analyze",
    "Analyze one exact mainland listed fund",
    "Validate one exact mainland ETF or listed-fund identity, share class, dated holdings, NAV and market facts, fees, rights and receipts, then perform caller-requested return and premium or discount calculations",
    "The caller owns fund selection and interpretation; no constituent inference, stale holding fill, liquidity judgment or recommendation occurs",
...
fn finish(
  ...
  case outcome {
    finance_local_import.Loaded(text, _) ->
      case domain.listed_fund(text, expected) {
        Ok(value) ->
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "CN listed-fund packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("CN listed-fund import was cancelled")
    finance_local_import.Missing ->
      tool.reject("CN listed-fund packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("CN listed-fund packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("CN listed-fund import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("CN listed-fund import returned an invalid effect result")
  }
}
