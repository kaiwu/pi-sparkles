...
import finance_multi_asset/options as domain
...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "options_analyze",
    "Analyze one exact option contract and strategy",
    "Validate one exact option identity, deliverable, quote, adjustment lineage and multi-leg expiration grid, then run a caller-selected bounded Black-Scholes or binomial model, implied volatility and finite-difference Greeks",
    "The caller owns model, inputs, bumps and interpretation; no probability, liquidity, hedge, value judgment or recommendation occurs",
...
fn finish(
  ...
  case outcome {
    finance_local_import.Loaded(text, _) ->
      case domain.analyze(text, expected) {
        Ok(value) ->
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "Options packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("Options import was cancelled")
    finance_local_import.Missing -> tool.reject("Options packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("Options packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("Options import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("Options import returned an invalid effect result")
  }
}
