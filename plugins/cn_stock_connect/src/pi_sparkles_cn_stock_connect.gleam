...
import pi/schema
import pi/tool
import pi_sparkles_cn_stock_connect/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_stock_connect",
    "Inspect Mainland Stock Connect facts",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_stock_connect_record",
    "Drill one Mainland Stock Connect facts record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_stock_connect; no latest-record or preferred-source choice is made",
