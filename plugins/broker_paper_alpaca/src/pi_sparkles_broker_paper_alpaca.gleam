...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    ...
    "Review offline Alpaca paper evidence",
    "Validate a bounded deterministic-scenario envelope or caller-owned Alpaca paper receipt with no network or broker authority; every result is track_partial",
...
fn review_schema() -> schema.Schema {
  schema.object([
    ...
    schema.Required(
      "environment",
      schema.string_enum(["local_simulation", "alpaca_paper"]),
    ),
