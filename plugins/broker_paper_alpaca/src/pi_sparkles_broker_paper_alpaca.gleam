...
import pi/schema
import pi/tool
import pi_sparkles_broker_paper_alpaca/domain

pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "review_alpaca_paper_evidence",
    "Review an external Alpaca paper receipt packet",
    "Validate bounded normalized Alpaca paper-account, order, fill, capability, entitlement, and lifecycle evidence from the explicit external read-only provider capability",
    "Alpaca is an external dependency. This plugin ships no SDK, credential, adapter, network transport, or broker mutation surface.",
...
fn register_simulation(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "simulate_alpaca_tape_possible_fill",
    "Simulate one Alpaca transaction-tape possible-fill scenario",
    "Apply the named provider-neutral transaction_tape_possible_fill_v1 model to one exact US limit instruction and bounded external Alpaca transaction tape, retaining eligible prints, exclusions, sequence limitations, and both non-fill and possible-fill branches",
    ...
    tool.parameters(
      tape_scenario_schema.input(schema.string_enum(["alpaca"]), ["us"], [
        "XNYS",
        "XNAS",
      ]),
    ...
    fn(_id, input, signal, _updates, _ctx) {
      case tool.is_cancelled(signal) {
        True -> tool.reject("Alpaca possible-fill simulation was cancelled")
        False ->
          case tape_scenario.run(input, "us", Some("alpaca")) {
            Ok(value) ->
              tool.text_result(
