...
import pi/schema
import pi/tool
import pi_sparkles_cn_stock_public_info/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_exchange_public_info",
    "Inspect CN exchange public information",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_exchange_public_info_record",
    "Drill one CN exchange public information record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_exchange_public_info; no latest-record or preferred-source choice is made",
