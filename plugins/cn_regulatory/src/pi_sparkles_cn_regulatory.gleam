...
import pi/schema
import pi/tool
import pi_sparkles_cn_regulatory/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_regulatory_documents",
    "Inspect CN regulatory documents",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_regulatory_record",
    "Drill one CN regulatory documents record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_regulatory_documents; no latest-record or preferred-source choice is made",
