...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "cn_ipo_research",
    "Research one exact mainland IPO packet",
    "Validate one exact mainland China IPO identity, append-only regulatory state history, offer facts, accounting context and caller-requested mechanical calculations with source, correction, rights and receipt lineage",
    "The caller owns source selection and interpretation; no approval prediction, allocation advice, valuation verdict or recommendation occurs",
...
fn finish(
  ...
  case outcome {
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "CN IPO packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled -> tool.reject("CN IPO import was cancelled")
    finance_local_import.Missing -> tool.reject("CN IPO packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("CN IPO packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("CN IPO import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("CN IPO import returned an invalid effect result")
  }
}
