...
Status: **Implemented inventory — `track_partial`** · caller-owned import review only · optional Alpaca network breadth absent
...
optional Alpaca network adapter. Futu is T6's required first US live provider;
Alpaca remains separately selected provider breadth and is never a fallback.
...
The live Alpaca network adapter is optional provider breadth outside T6's
required Futu-first US leg. Fixture decoding and a caller-owned account-activity
export may prove the bounded import contract; any network mode requires
reviewed rights and a demonstrably read-only key. No write-capable credential,
