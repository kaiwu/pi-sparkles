...
import finance_multi_asset/dashboard as domain
...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "macro_dashboard_compose",
    "Compose one exact multi-source macro dashboard",
    "Compose caller-supplied macro panels at a declared knowledge cutoff with native geography, frequency and units, explicit date intersection, selected transforms, source handoffs and least-permissive rights inventory",
    "The caller owns panel, transform and interpretation choices; no direct acquisition, imputation, frequency coercion, forecast or policy conclusion occurs",
...
fn finish(
  ...
  case outcome {
    finance_local_import.Loaded(text, _) ->
      case domain.dashboard(text, expected) {
        Ok(value) ->
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "Macro-dashboard packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("Macro-dashboard import was cancelled")
    finance_local_import.Missing ->
      tool.reject("Macro-dashboard packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("Macro-dashboard packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("Macro-dashboard import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("Macro-dashboard import returned an invalid effect result")
  }
}
