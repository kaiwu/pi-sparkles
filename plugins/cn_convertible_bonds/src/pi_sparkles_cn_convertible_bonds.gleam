...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "cn_convertible_analyze",
    "Analyze one exact mainland convertible bond",
    "Validate one exact mainland convertible-bond identity, terms, adjustment lineage, source facts and caller-declared call, put, hold and conversion scenarios with mechanical conversion calculations",
    "The caller owns scenario assumptions and interpretation; no exercise, call-risk forecast, trading advice or recommendation occurs",
...
fn finish(
  ...
  case outcome {
    finance_local_import.Loaded(text, _) ->
      case domain.convertible(text, expected) {
        Ok(value) ->
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "CN convertible-bond packet exceeds maximumBytes: "
        <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("CN convertible-bond import was cancelled")
    finance_local_import.Missing ->
      tool.reject("CN convertible-bond packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("CN convertible-bond packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("CN convertible-bond import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject(
        "CN convertible-bond import returned an invalid effect result",
      )
  }
}
