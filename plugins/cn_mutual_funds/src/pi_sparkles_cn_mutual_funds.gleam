...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "cn_mutual_fund_analyze",
    "Analyze one exact mainland mutual-fund share class",
    "Validate one exact mainland mutual-fund and share-class identity, dated holdings, NAV history, fee facts, rights and receipts, then perform caller-requested return and fee-drag calculations",
    "The caller owns share-class selection and interpretation; no listed-market proxy, stale holding fill, performance verdict or recommendation occurs",
...
fn finish(
  ...
  case outcome {
    finance_local_import.Loaded(text, _) ->
      case domain.mutual_fund(text, expected) {
        Ok(value) ->
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "CN mutual-fund packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("CN mutual-fund import was cancelled")
    finance_local_import.Missing ->
      tool.reject("CN mutual-fund packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("CN mutual-fund packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("CN mutual-fund import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("CN mutual-fund import returned an invalid effect result")
  }
}
