...
pub fn extension(api: pi.ExtensionApi) -> Promise(Nil) {
  tool.register(
    api,
    "commodities_analyze",
    "Analyze one exact commodity futures curve",
    "Validate exact futures contract identities, specifications, dated curve facts and source receipts, then calculate caller-selected calendar and inter-commodity spreads and an explicit named roll artifact",
    "The caller owns spread legs, roll method and interpretation; no anonymous continuous contract, silent FX, structure label, storage inference, forecast or recommendation occurs",
...
fn finish(
  ...
  case outcome {
    finance_local_import.Loaded(text, _) ->
      case domain.commodities(text, expected) {
        Ok(value) ->
    ...
    finance_local_import.Truncated(_, total) ->
      tool.reject(
        "Commodities packet exceeds maximumBytes: " <> int.to_string(total),
      )
    finance_local_import.Cancelled ->
      tool.reject("Commodities import was cancelled")
    finance_local_import.Missing ->
      tool.reject("Commodities packet was not found")
    finance_local_import.InvalidUtf8 ->
      tool.reject("Commodities packet requires strict UTF-8")
    finance_local_import.Failure(code) ->
      tool.reject("Commodities import failed safely: " <> code)
    finance_local_import.InvalidResult ->
      tool.reject("Commodities import returned an invalid effect result")
  }
}
