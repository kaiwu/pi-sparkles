...
import pi/schema
import pi/tool
import pi_sparkles_hk_stock_connect/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "hk_stock_connect",
    "Inspect HK Stock Connect facts",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "hk_stock_connect_record",
    "Drill one HK Stock Connect facts record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by hk_stock_connect; no latest-record or preferred-source choice is made",
