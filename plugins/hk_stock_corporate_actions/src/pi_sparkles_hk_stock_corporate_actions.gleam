...
import pi/schema
import pi/tool
import pi_sparkles_hk_stock_corporate_actions/domain
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "hk_corporate_action_record",
    "Drill one HK corporate actions record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by hk_corporate_actions; no latest-record or preferred-source choice is made",
