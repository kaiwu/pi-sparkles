...
import pi/schema
import pi/tool
import pi_sparkles_cn_stock_insiders/domain
...
fn register_inspect(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_insider_disclosures",
    "Inspect CN insider and major-holder disclosures",
...
fn register_drill(api: pi.ExtensionApi) -> Nil {
  tool.register(
    api,
    "cn_insider_record",
    "Drill one CN insider and major-holder disclosures record",
    "Reread the same exact content-bound import and return one complete record with source fields and correction lineage",
    "Use the packet hash and recordId returned by cn_insider_disclosures; no latest-record or preferred-source choice is made",
